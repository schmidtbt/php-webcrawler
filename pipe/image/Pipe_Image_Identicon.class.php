 ...
 * @author Revan
 */
class Pipe_Image_Identicon extends Pipe_Image  {
    
    protected $_identiconPath;
    protected static $_spritez = 128;
    
    
    public function __construct( $keyString ) {
        $this->_img = $this->genIdenticon( md5($keyString) );
        $this->_identiconPath = $this->genTempImage( $this->_img );
    }
    
    public function getIdenticonPath(){
        return $this->_identiconPath;
    }
    
    function genIdenticon( $hash, $size = 100 ){
        ...
        $identicon=imagecreatetruecolor(static::$_spritez*3,static::$_spritez*3);
        imageantialias($identicon,TRUE);

        /* assign white as background */
        $bg=imagecolorallocate($identicon,255,255,255);
        imagefilledrectangle($identicon,0,0,static::$_spritez,static::$_spritez,$bg);

        /* generate corner sprites */
        $corner=$this->getsprite($csh,$cfr,$cfg,$cfb,$cro);
        imagecopy($identicon,$corner,0,0,0,0,static::$_spritez,static::$_spritez);
        $corner=imagerotate($corner,90,$bg);
        imagecopy($identicon,$corner,0,static::$_spritez*2,0,0,static::$_spritez,static::$_spritez);
        $corner=imagerotate($corner,90,$bg);
        imagecopy($identicon,$corner,static::$_spritez*2,static::$_spritez*2,0,0,static::$_spritez,static::$_spritez);
        $corner=imagerotate($corner,90,$bg);
        imagecopy($identicon,$corner,static::$_spritez*2,0,0,0,static::$_spritez,static::$_spritez);

        /* generate side sprites */
        $side=$this->getsprite($ssh,$sfr,$sfg,$sfb,$sro);
        imagecopy($identicon,$side,static::$_spritez,0,0,0,static::$_spritez,static::$_spritez);
        $side=imagerotate($side,90,$bg);
        imagecopy($identicon,$side,0,static::$_spritez,0,0,static::$_spritez,static::$_spritez);
        $side=imagerotate($side,90,$bg);
        imagecopy($identicon,$side,static::$_spritez,static::$_spritez*2,0,0,static::$_spritez,static::$_spritez);
        $side=imagerotate($side,90,$bg);
        imagecopy($identicon,$side,static::$_spritez*2,static::$_spritez,0,0,static::$_spritez,static::$_spritez);

        /* generate center sprite */
        $center=$this->getcenter($xsh,$cfr,$cfg,$cfb,$sfr,$sfg,$sfb,$xbg);
        imagecopy($identicon,$center,static::$_spritez,static::$_spritez,0,0,static::$_spritez,static::$_spritez);
        ...
        imagecopyresampled($resized,$identicon,0,0,(imagesx($identicon)-static::$_spritez*3)/2,(imagesx($identicon)-static::$_spritez*3)/2,$size,$size,static::$_spritez*3,static::$_spritez*3);
...
function getsprite($shape,$R,$G,$B,$rotation) {
	$sprite=imagecreatetruecolor(static::$_spritez,static::$_spritez);
	imageantialias($sprite,TRUE);
	$fg=imagecolorallocate($sprite,$R,$G,$B);
	$bg=imagecolorallocate($sprite,255,255,255);
	imagefilledrectangle($sprite,0,0,static::$_spritez,static::$_spritez,$bg);
	switch($shape) {
		case 0: // triangle
			$shape=array(
				0.5,1,
	...
	for ($i=0;$i<count($shape);$i++)
		$shape[$i]=$shape[$i]*static::$_spritez;
